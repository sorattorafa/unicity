...
import { getToken, getUserId, getStatus } from '../services/auth';
...
class CreateAdminUser extends React.Component {
  ...
  componentWillMount() {
    axios.get('/adminusers/id/' + getUserId())
        .then(response => {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        axios.put('/adminusers/' + this.state.id, values)
            .then(res => {
                ...
                <Row className="buttonForm">
                  <Col span={24} style={{ textAlign: 'right' }}>
                    <Form.Item>
                      ...
                      <Link style = {{paddingLeft: "5px"}} to = { "/listusers"} >
